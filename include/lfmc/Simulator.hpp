 ...
 */
template <StochasticProcess P, typename S, typename Payoff>
class Simulator {
public:
    ...
    Simulator(P process, S scheme, Payoff payoff, double x0, double T, size_t n_steps, size_t n_simulations)
        : process_(std::move(process)), scheme_(std::move(scheme)),
          payoff_(std::move(payoff)), x0_(x0), T_(T), n_steps_(n_steps),
          n_simulations_(n_simulations) {}
        // TODO initialize other members
    


        //Used Claude to develope this sim. didn't feel like putting in the work since it will be replaced later...
