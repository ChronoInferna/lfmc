...
double geometric_asian_call(double S, double K, double r,
                             double sigma, double T, int n) {
    double sigma_adj = sigma * std::sqrt((2.0 * n + 1.0) / (6.0 * (n + 1.0)));
    double r_adj     = 0.5 * (r - 0.5 * sigma * sigma)
    ...
    return std::exp(-r * T)
           * (S * std::exp(r_adj * T) * norm_cdf(d1) - K * norm_cdf(d2));
}

// Up-and-Out Call closed form (continuous barrier, no dividends)
double up_and_out_call(double S, double K, double H,
                        double r, double sigma, double T) {
    if (S >= H) return 0.0;
    double vanilla = european_call(S, K, r, sigma, T);

    double lambda = (r + 0.5 * sigma * sigma) / (sigma * sigma);
    double d1 = (std::log(H * H / (S * K)) + (r + 0.5 * sigma * sigma) * T)
                / (sigma * std::sqrt(T));
    double d2 = d1 - sigma * std::sqrt(T);
    double reflection = std::pow(H / S, 2.0 * lambda)
...
std::vector<ConvergenceResult> run_convergence(
    const std::string&    label,
    PayoffFactory         make_payoff,
    double                ground_truth,
    const std::vector<std::size_t>& sample_tiers,
    double S0    = 100.0,
    double mu    = 0.05,
    double sigma = 0.20,
    ...
    double T     = 1.0)
{
    ...
    for (std::size_t n : sample_tiers) {
        ...
        auto pg  = std::make_unique<
            PathGenerator<GeometricBrownianMotion,
                          EulerMaruyama<GeometricBrownianMotion>>>(gbm, euler);
        auto po  = make_payoff();
        auto est = std::make_unique<MonteCarloEstimator>(n); // see note below

        Pipeline<GeometricBrownianMotion,
                 EulerMaruyama<GeometricBrownianMotion>> pipeline(
            std::move(rs), std::move(pg),
        ...
        std::cout << std::setw(10) << n
                  << std::setw(14) << std::fixed << std::setprecision(4) << estimate
                  << std::setw(14) << ground_truth
                  << std::setw(14) << err
                  << std::setw(12) << elapsed
                  << "\n";
    }
...
static const std::vector<std::size_t> TIERS =
...
TEST_CASE("Asian Call convergence", "[exotic][convergence][asian]") {
    ...
    auto results = run_convergence(
        ...
        []() { return std::make_unique<AsianCall>(K); },
        truth, TIERS);
...
TEST_CASE("Up-and-Out Barrier Call convergence",
          "[exotic][convergence][barrier]") {
    double truth = bs::up_and_out_call(S0, K, B_UP, MU, SIGMA, T);

    auto results = run_convergence(
        "Up-and-Out Barrier Call",
        []() { return std::make_unique<UpAndOutCall>(K, B_UP); },
        truth, TIERS);
...
TEST_CASE("Lookback Call convergence", "[exotic][convergence][lookback]") {
    ...
    auto results = run_convergence(
        ...
        []() { return std::make_unique<LookbackCall>(); },
    ...
    auto spread = [&](int a, int b) {
        return std::abs(results[b].estimate - results[a].estimate);
    };
    REQUIRE(spread(3, 5) < spread(0, 2));
}

TEST_CASE("Sanity check: European Call matches Black-Scholes",
          "[sanity][european]") {
    double bs_price = bs::european_call(S0, K, MU, SIGMA, T);

    auto results = run_convergence(
        "European Call (BS sanity check)",
        []() { return std::make_unique<EuropeanCall>(K); },
        bs_price, TIERS);
