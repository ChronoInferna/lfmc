...
static std::string abbrev(const std::string& name) {
    ...
    if (name == "halton_qmc")
        return "hal";
    if (name == "importance_sampling")
        return "imp";
    if (name == "moment_matching")
        return "mm";
    if (name == "lhs")
        return "lhs";
...
int main() {
    ...
    for (auto& or_ : all_results) {
        ...
        for (size_t ci = 0; ci < N_COUNTS; ++ci) {
            ...
            snprintf(best_buf, sizeof(best_buf), "%s:%.1fx", abbrev(r.best_fixed_name).c_str(),
                     best_ratio);
    ...
    for (auto& or_ : all_results) {
        ...
        printf("  %-16s  %.1fx → %.1fx  (%+.1f%%)  %s\n", or_.name, ratio15, ratio100,
               ratio100 - ratio15, flag);
    }
    ...
    for (auto& or_ : all_results) {
        printf("\n%s\n", or_.name);
        printf("  %-8s  %12s  %12s  %12s  %8s  %-16s  %8s\n", "Runs", "Engine MSE", "PlainMC MSE",
               "BestFix MSE", "Eng/plain", "BestFix", "Gap");
        for (size_t ci = 0; ci < N_COUNTS; ++ci) {
            const auto& r = or_.results[ci];
            double eng_ratio = r.plain_mse / r.engine_mse;
            double best_ratio = r.plain_mse / r.best_fixed_mse;
            ...
            printf("  %-8zu  %12.6f  %12.6f  %12.6f  %8.2fx  %-16s  %+.2fx\n", RUN_COUNTS[ci],
                   r.engine_mse, r.plain_mse, r.best_fixed_mse, eng_ratio,
                   (abbrev(r.best_fixed_name) + ":" + std::to_string(best_ratio).substr(0, 4) + "x")
                       .c_str(),
                   gap);
        }
    }
    ...
    for (auto& or_ : all_results) {
        if (std::string(or_.name).find("Asian") == std::string::npos)
            continue;
        for (size_t ci = 0; ci < N_COUNTS; ++ci) {
