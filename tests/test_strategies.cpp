...
TEST_CASE("strategies: all 10 strategies are unbiased for European call") {
    ...
    for (const auto& s : strategies) {
        ...
        INFO("Strategy: " << s.name << "  mean=" << mean << "  ref=" << ref
                          << "  5-sigma=" << 5.0 * se);
...
TEST_CASE("IS: theta sensitivity for call and put - VR ratio study") {
    ...
    for (double theta : thetas) {
        auto is_call =
            make_importance_sampler(GBM, EULER, std::make_shared<EuropeanCall>(K), STEPS, T, theta);
        auto is_put =
            make_importance_sampler(GBM, EULER, std::make_shared<EuropeanPut>(K), STEPS, T, theta);
...
static void log_result(const ASVRResult& r) {
    for (size_t k = 0; k < r.exploration_stats.size(); ++k) {
        INFO("  " << r.exploration_stats[k].name << ": var="
                  << r.exploration_stats[k].sample_variance << " weight=" << r.precision_weights[k]
                  << " n_exploit=" << r.exploitation_counts[k]);
    }
    INFO("VR ratio: " << r.variance_reduction_ratio);
    INFO("Best strategy: " << r.exploration_stats[std::max_element(r.precision_weights.begin(),
                                                                   r.precision_weights.end()) -
                                                  r.precision_weights.begin()]
                                  .name);
}
...
TEST_CASE("strategies: empirical MSE comparison across all option types", "[.slow]") {
    ...
    INFO("\n| Option Type                     | Plain MC MSE | ASVR MSE  | VR Ratio | Best "
         "Strategy       |");
    INFO("|---------------------------------|-------------|-----------|----------|-----------------"
         "----|");

    auto plain_mc =
        make_plain_mc_sampler(GBM, EULER, nullptr, STEPS, T); // will be rebuilt per option
    ...
    for (auto& [opt_name, payoff] : options) {
        ...
        for (size_t r = 0; r < RUNS; ++r) {
            ...
            if (r == 0) {
                const size_t best_k =
                    static_cast<size_t>(std::max_element(asvr_res.precision_weights.begin(),
        ...
        INFO("| " << opt_name << " | " << mse_plain << " | " << mse_asvr << " | " << vr << " | "
                  << best_name << " |");
